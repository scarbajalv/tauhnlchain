// ./X idGun mHNL.dat maxoffaxis deltaoffaxis seed nroot index"
     
// REQUISITOS:
// 1. export mainconfig = "/home/user/mainhnlchain/mainconfig/"
//		Aquí se encuentran los archivos de energias y angulos de los mesones.
// 2. export hnldata = "/home/user/mainhnlchain/data/"
//		Aquí se almacena la data. La estructura debe ser de la forma
//			data/dirac/meson/411	
// 		Repitiendo para dirac/majorana, meson/tau, 411/431/411bar/431bar
// 3. En MAIN PARAMETERS fijar la naturaleza del HNL (dirac/majorana)
// 		y de su padre (meson/tau).
// 4.	Este archivo debe estar junto a la carpeta que contiene los mathbrs. 
//		También debe estar junto a la carpeta mainconfig que contiene
//		pythiaconfig.ini. Falta pulir esto para que no sea necesario.
...
int main(int argc, char *argv[]) {
	...
	if(argc<7){
		cout<<"FATAL ERROR: ./X idGun mHNL.dat maxoffaxis deltaoffaxis seed nroot index"
				<<	endl;
		exit (EXIT_FAILURE);
	}
  ...
  nroot_ss  << "/home/sane/mainhnlchain/data/taudecaytest/" <<"120-"<<argv[6]<<".root";
	...
	idata_ss << getEnv("mainconfig") << "d" << idGun_s << "data-120-" << argv[7] << ".dat";	
  ...
  cout<<"HNL Nature = "<< hnlNature <<endl;
  cout<<"HNL mother = "<< hnlMother <<endl;
...
// ************************ MAIN LOOP ********************************
  ...
  if((iEvent+1) % 10000 == 0){
	 cout<<"\r"<<fixed<<setprecision(2)<<(iEvent+1)/1000000.<<" M"<<std::flush;
	}
