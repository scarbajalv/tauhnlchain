...
// ./X idGun maxoffaxis deltaoffaxis seed nroot index
...
int main(int argc, char *argv[]) {
	...
	if(argc<6){
		cout<<"FATAL ERROR: ./X idGun maxoffaxis deltaoffaxis seed nroot index"<<endl;
		exit (EXIT_FAILURE);
	}
  ...
  stringstream nroot_ss;
	nroot_ss  << getEnv("tauhnldata") << "bg/" << idGun_s << "/" 
            << idGun_s << "-"<<argv[5]<<".root";
  ...
  char nroot_c[nroot_s.size()+1];
	...
	idata_ss << getEnv("mainconfig") <<"d"<<idGun_s<<"data-120-" << argv[6] << ".dat"; 
