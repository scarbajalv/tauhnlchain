...
int main(){
  ...
  double	xidet = (-lmpd/2+offaxis)*unitfactor, 
					xfdet = (lmpd/2+offaxis)*unitfactor, 
          radio = rmpd * unitfactor,
          cy = 0,
          cz = (z0det + l + rmpd)*unitfactor,
          yidet = (-rmpd)*unitfactor,
          yfdet = (rmpd)*unitfactor, 
          zidet = (z0det + l)*unitfactor, 
          zfdet = (z0det + l + 2*rmpd)*unitfactor;
  ...
  for(int i=0; i<nevents; ++i){
    ...
    while(incircle == false){
      xx[1] = gsl_ran_flat(r,yidet,yfdet);
      xx[2] = gsl_ran_flat(r,zidet,zfdet);
      if(pow(xx[1]-cy,2)+pow(xx[2]-cz,2) < pow(radio,2)){
        incircle = true;
      }
    }
